...
ggplot(top10.hisp.counties,
  ...
  coord_flip()+
  labs(
    title="Top 10 NY Counties: Sepsis Rates (Hispanic, 2016–2018)",
    x="County",
...
#library maps uses lower case for county sepsis data set uses upper case
...
#maps uses lowercase county names 
#our data uses lower case 

NY.Hispanic_joined<-NY%>%
  left_join(
    hispanic_sepsis.rate%>%
              mutate(
                Patient_County=tolower(Patient_County),
                Patient_County = gsub("saint", "st", Patient_County)),
            by=c("subregion"="Patient_County")
            )
            
ggplot(NY.Hispanic_joined,
  ...
  labs(
    title="Hispanic Maternal Sepsis Hotspots(SPARCS 2016–2018)",
    fill="/100k Births"
  )+
  theme_void()
#Notes
...
#MAP GEO WORK AI USE DISCLOSURE
#This work was completed with the assistance of ChatGPT (OpenAI), which was used to support R coding, data cleaning, and geospatial visualization. All analysis decisions, interpretation of results, and final outputs were reviewed and verified by the author.
